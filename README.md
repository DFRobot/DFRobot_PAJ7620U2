...
```C++
   ...
   * @n  In fast detection mode, the sensor can recognize 9 gestures: move left, right, up, down,forward, backward, clockwise,
   * @n  counter-clockwise, wave.  
   ...
   * @n
   * @n
   * @n  In slow detection mode, the sensor recognize one gesture every 2 seconds, and we have integrated the expanded gestures inside      * @n  the library, which is convenient for the beginners to use.  
   * @n  The slow mode can recognize 9  basic gestures and 4 expanded gestures: move left, right, up, down, forward, backward,      
   * @n  clockwise, counter-clockwise, wave, slowly move left and right, slowly move up and down, slowly move forward and backward, wave
   * @n  slowly and randomly
   */
   ...
   * @return Textual description corresponding to the gesture number:if the gesture input in the gesture table doesn't exist, return        * @n null string 
